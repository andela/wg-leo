...
class WorkoutLogUpdateView(WgerFormMixin, UpdateView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context[
            'title'] = _(u'Edit log entry for %s') % self.object.exercise.name
...
def add(request, pk):
    ...
    # and repetitions have no indicator to which
    ...
    if request.method == 'POST':
        ...
        # The reason for this is that the form expects a value for
        ...
        if dateform.is_valid() and session_form.is_valid(
            ...
            # Save the Workout Session only
            # if there is not already one for this date
        ...
        for day_list in self.object.canonical_representation['day_list']:
            ...
            for set_list in day_list['set_list']:
                ...
                for exercise_list in set_list['exercise_list']:
                    ...
                    #       when adding the filter there, the execution
                    #       time explodes. The weight unit filter works as
                    #       expected. Also, adding the unit IDs to the exclude
                    #       list also has the disadvantage that if new
                    #       ones are added in a
