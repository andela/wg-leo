...
def load_ingredient_languages(request):
    ...
    Additionally, if the user has selected on his
    preference page that he wishes
    to also see the ingredients in English
    (from the US Department of Agriculture), show those too.
    ...
    if not date:
        ...
        p = Paragraph(
            '''<para>
                ...
                date=date, url=url, version=get_version()),
            styleSheet["Normal"])
    return p
